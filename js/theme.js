...
(function($) {
    ...
    $('#flightTravellersClass').on('click', function() {
        ...
        $('.qty-spinner, .flight-class').on('change', function() {
            var ids = ['flightAdult', 'flightChildren', 'flightInfants', 'flightOgrenci'];
    ...
    /* Flight select @anilsahindev */
    var elmContinueBtn = $('.contiune-button');
    var elmSearchBtn = $('.search-button');
    if (window.outerWidth < 768) {
        elmContinueBtn.removeClass('m-3');
        $('#bookingFlight div').hide();
        $('.search-section').show();
        elmSearchBtn.show();
        elmSearchBtn && elmSearchBtn.on('click', function() {
            $('#bookingFlight div').show();
        });
    }


    var elmReturnFlight = $('.return-flight');
    elmReturnFlight && elmReturnFlight.hide();
    elmContinueBtn && elmContinueBtn.hide();
    var elmDepartureFlight = $('.departure-flight');
    elmDepartureFlight && elmDepartureFlight.on('click', '.flight-item', function() {
        $('.gidis-ucus-baslik').show();
        if (window.outerWidth > 768) {
            elmDepartureFlight.css('padding-right', '0');
            elmReturnFlight.css('padding-left', '0');
        }
        elmDepartureFlight.removeClass('col-lg-12');
        elmDepartureFlight.addClass('col-lg-6');
        elmReturnFlight.show();
        elmReturnFlight.addClass('col-lg-6');
        $('.departure-flight .flight-item').css('display', 'none');
        $(this).css('display', 'block');
    });
    elmReturnFlight && elmReturnFlight.on('click', '.flight-item', function() {
        $('.return-flight .flight-item').css('display', 'none');
        $(this).css('display', 'block');
        elmContinueBtn && elmContinueBtn.show();
    });
